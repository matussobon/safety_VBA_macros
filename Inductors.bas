...
Set ws1 = Worksheets("Coils")

For i = 1 To Range("A1000").End(xlUp).Row - 2
Range("B" & i + 2).Value = WorksheetFunction.VLookup(Range("A" & i + 2).Value, ws1.Range("A3:W5000"), 18, 0) 'treba zmenit cislo stlpca v ktorom je part failure rate
...
For j = 1 To Range("C1000").End(xlUp).Row - 2
    ...
    If InStr(Range("C" & j + 2).Value, "Inductor") <> 0 Then
        inductor = inductor + Range("B" & j + 2).Value

    ElseIf InStr(Range("C" & j + 2).Value, "Transformer") <> 0 Then
        transformer = transformer + Range("B" & j + 2).Value
    
    ElseIf InStr(Range("C" & j + 2).Value, "Suppression") <> 0 Then
        suppression = suppression + Range("B" & j + 2).Value
    End If
...
For k = 1 To Range("A1000").End(xlUp).Row - 2
    ...
    If InStr(Range("A" & k + 2).Value, "short") <> 0 Then
        vysledok = (0.5 * inductor) + (0.5 * transformer) + (0.5 * suppression)
    ElseIf InStr(Range("A" & k + 2).Value, "open") <> 0 Then
        vysledok = (0.5 * inductor) + (0.5 * transformer) + (0.5 * suppression)
    ElseIf InStr(Range("A" & k + 2).Value, "failure") <> 0 Then
        vysledok = inductor + transformer + suppression
    End If
...
Sub W_CALC()
Attribute W_CALC.VB_ProcData.VB_Invoke_Func = "a\n14"
