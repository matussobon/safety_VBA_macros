...
Set ws1 = Worksheets("Resistors")

For i = 1 To Range("A1000").End(xlUp).Row - 2
Range("B" & i + 2).Value = WorksheetFunction.VLookup(Range("A" & i + 2).Value, ws1.Range("A3:W5000"), 23, 0) 'tu treba zmenit cislo stlpca v ktorom je part failure rate
...
For j = 1 To Range("C1000").End(xlUp).Row - 2
    ...
    If InStr(Range("C" & j + 2).Value, "Film") <> 0 Then

        film = film + Range("B" & j + 2).Value

    ElseIf InStr(Range("C" & j + 2).Value, "Other") <> 0 Then
        other = other + Range("B" & j + 2).Value
...
Sub R_CALC()
Attribute R_CALC.VB_ProcData.VB_Invoke_Func = "x\n14"
